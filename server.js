...
<head>
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    /* ── NETWORKS ── */
    .ab-net-grid { display: grid; grid-template-columns: 1fr; gap: 14px; }
    .ab-net-card {
      background: var(--card);
      border: 2.5px solid var(--border);
      border-radius: 24px; padding: 1.4rem 1.8rem; text-align: center;
      position: relative; overflow: hidden;
    ...
    .ab-net-card:hover { border-color: rgba(201,162,39,.4); transform: translateY(-3px); box-shadow: 0 16px 48px rgba(17,17,17,.1), inset 0 1px 0 rgba(255,255,255,.9), 0 0 24px rgba(201,162,39,.08); }
    .ab-net-logo {
      width: 56px; height: 56px; border-radius: 16px; margin: 0; flex-shrink: 0;
      display: flex; align-items: center; justify-content: center; font-size: 24px;
      box-shadow: 0 6px 20px rgba(0,0,0,.1);
    }
    .ab-net-logo-bnb {
      position: relative;
      background: radial-gradient(circle, rgba(240,185,11,.28) 0%, rgba(240,185,11,.08) 65%, transparent 100%);
      box-shadow: 0 0 24px rgba(240,185,11,.4), 0 6px 20px rgba(0,0,0,.12);
    }
    .ab-net-logo-bnb::after {
      content: ''; position: absolute; inset: -8px; border-radius: 22px; z-index: -1;
      background: radial-gradient(circle, rgba(240,185,11,.35), transparent 70%);
      filter: blur(9px);
    }
    .ab-net-logo-bnb svg { filter: drop-shadow(0 2px 5px rgba(240,185,11,.55)); }
    .ab-net-info { text-align: left; }
    .ab-net-name { font-size: 16px; font-weight: 800; color: var(--black); margin-bottom: 2px; letter-spacing: .01em; }
    .ab-net-sub { font-size: 11.5px; color: var(--muted); margin-bottom: 0; }
    .ab-net-desc { font-size: 11px; color: var(--muted2); line-height: 1.5; text-align: left; margin: 0; padding: 0 0 0 1.6rem; border-left: 1px solid var(--border); flex: 1; min-width: 220px; }
    .ab-net-badge {
      display: inline-flex; align-items: center; gap: 5px; flex-shrink: 0;
      background: rgba(34,197,94,.1); border: 1px solid rgba(34,197,94,.3);
      border-radius: 999px; padding: 5px 14px;
      font-size: 10.5px; font-weight: 700; color: #16a34a; letter-spacing: .06em;
    }
    .ab-net-dot { width: 6px; height: 6px; border-radius: 50%; background: #22c55e; animation: ab-glow 1.5s ease infinite; }
    @media (max-width: 640px) {
      .ab-net-card { flex-direction: column; text-align: center; }
      .ab-net-info { text-align: center; }
      .ab-net-desc { border-left: none; border-top: 1px solid var(--border); padding: .9rem 0 0; text-align: center; }
    }
    ...
    /* Landing page overrides dark body — must have own warm background */
    #page-landing {
      background: linear-gradient(165deg,#FAF9F6 0%,#F6F5F1 35%,#F2F1EC 65%,#F6F5F1 100%) !important;
    }
    ...
    /* ══ LANDING PAGE ══ */
    @keyframes lp-fl  { 0%,100%{transform:translateY(0)}  50%{transform:translateY(-10px)} }
    @keyframes lp-fls { 0%,100%{transform:translateY(0)}  50%{transform:translateY(-6px)}  }
    @keyframes lp-rot { from{transform:translate(-50%,-50%) rotate(0deg)} to{transform:translate(-50%,-50%) rotate(360deg)} }
    @keyframes lp-rtr { from{transform:translate(-50%,-50%) rotate(0deg)} to{transform:translate(-50%,-50%) rotate(-360deg)} }
    @keyframes lp-gc  { 0%,100%{opacity:.6;transform:translate(-50%,-50%) scale(1)} 50%{opacity:.9;transform:translate(-50%,-50%) scale(1.05)} }
    @keyframes lp-btn { 0%,70%,100%{box-shadow:0 6px 28px rgba(201,162,39,.45)} 80%{box-shadow:0 10px 48px rgba(201,162,39,.8)} }
    @keyframes lp-logo{ 0%,100%{transform:translateY(0)} 50%{transform:translateY(-3px)} }
    @keyframes shd-float  { 0%,100%{transform:translateY(0) rotate(0deg)}  30%{transform:translateY(-8px) rotate(2deg)}  70%{transform:translateY(-4px) rotate(-1.5deg)} }
    @keyframes shd-glow   { 0%,100%{filter:drop-shadow(0 6px 14px rgba(180,140,20,.3)) drop-shadow(0 0 0px rgba(255,220,60,0))} 50%{filter:drop-shadow(0 14px 32px rgba(201,162,39,.7)) drop-shadow(0 0 18px rgba(255,220,60,.4))} }
    @keyframes shd-check  { 0%,80%,100%{stroke-dashoffset:60} 40%{stroke-dashoffset:0} }
    @keyframes shd-shine  { 0%,100%{opacity:0;transform:translateX(-40%) skewX(-15deg)} 40%,60%{opacity:.7;transform:translateX(140%) skewX(-15deg)} }
    @keyframes lp-btcs{ from{transform:rotateY(0deg)} to{transform:rotateY(360deg)} }
    @keyframes lp-bgl { 0%,100%{box-shadow:0 0 28px rgba(240,165,0,.4)} 50%{box-shadow:0 0 52px rgba(240,165,0,.72)} }
    @keyframes lp-swp { 0%{left:-35%;opacity:0} 20%{opacity:.7} 80%{opacity:.5} 100%{left:120%;opacity:0} }
    @keyframes lp-stk { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-8px)} }
    @keyframes lp-chr { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-9px)} }
    @keyframes lp-pt  { 0%{opacity:0;transform:translate(0,0)} 25%{opacity:.35} 75%{opacity:.2} 100%{opacity:0;transform:translate(var(--tx),var(--ty))} }

    /* page shell */
    #page-landing { background:#F7F6F2 !important; }
    .lp-wrap {
      position:absolute; inset:0;
      overflow:hidden;
      background:radial-gradient(ellipse 80% 70% at 50% 35%, #FCFBF9 0%, #F7F6F2 50%, #F0EFE9 100%);
    }
    ...
    .lp-cg {
      ...
      background:radial-gradient(circle, rgba(255,250,225,.7) 0%, rgba(248,244,235,.32) 50%, transparent 70%);
      animation:lp-gc 14s ease-in-out infinite;
    ...
    .lp-rg1 { width:min(340px,66vw); height:min(340px,66vw); border:1px solid rgba(201,162,39,.2); animation:lp-rot 28s linear infinite; }
    .lp-rg2 { width:min(460px,86vw); height:min(460px,86vw); border:1px solid rgba(201,162,39,.13); animation:lp-rtr 32s linear infinite; }
    .lp-rg3 { width:min(580px,104vw); height:min(580px,104vw); border:1px solid rgba(201,162,39,.07); animation:lp-rot 35s linear infinite; }

    /* tiny drifting particles */
    .lp-pts { position:absolute; inset:0; pointer-events:none; overflow:hidden; }
    .lp-pt  { position:absolute; border-radius:50%; opacity:0; background:#C9A227;
      animation:lp-pt var(--d,24s) ease-in-out var(--dl,0s) infinite; will-change:transform,opacity; }
    ...
    /* logo */
    .lp-logo-box {
      width:68px; height:68px; border-radius:20px; margin-bottom:.85rem;
      background:linear-gradient(150deg,#fff,#FAF6EE);
      border:1.5px solid rgba(201,162,39,.28);
      display:flex; align-items:center; justify-content:center;
      box-shadow:0 8px 28px rgba(201,162,39,.16), 0 2px 8px rgba(0,0,0,.06), inset 0 1px 0 #fff;
      animation:lp-logo 12s ease-in-out infinite; position:relative;
    }
    .lp-logo-box::before { content:''; position:absolute; inset:-1px; border-radius:21px; background:linear-gradient(135deg,rgba(201,162,39,.28),transparent 55%,rgba(201,162,39,.1)); z-index:-1; }

    /* brand */
    .lp-brand  { font-size:26px; font-weight:900; letter-spacing:.07em; line-height:1; margin-bottom:4px; }
    .lp-brand .q { color:#111; }
    .lp-brand .g { color:#C9A227; }
    .lp-tag    { font-size:9.5px; letter-spacing:.2em; color:#9B7A10; font-weight:700; text-transform:uppercase; opacity:.78; }

    /* divider */
    .lp-div { display:flex; align-items:center; gap:9px; width:120px; margin:.75rem auto .9rem; }
    .lp-div-l { flex:1; height:1px; background:linear-gradient(90deg,transparent,rgba(201,162,39,.4),transparent); }
    .lp-div-d { width:5px; height:5px; background:#C9A227; transform:rotate(45deg); flex-shrink:0; }

    /* headline */
    .lp-h1 { font-family:'Playfair Display',serif; font-size:27px; font-weight:700; color:#111; line-height:1.22; margin-bottom:.5rem; margin-top:1.5rem; }
    .lp-h1 em { color:#C9A227; font-style:italic; }
    .lp-p  { font-size:12.5px; color:#888; line-height:1.65; max-width:235px; margin:0 auto; }

    /* button */
    .lp-btn {
      display:inline-flex; align-items:center; gap:9px;
      padding:14px 36px; border-radius:50px; border:none; cursor:pointer;
      font-family:inherit; font-size:14.5px; font-weight:800; letter-spacing:.04em;
      color:#1A0E00; margin-top:0; position:relative; overflow:hidden;
      background:linear-gradient(135deg,#EDD040,#C9A227 38%,#E8C84A 62%,#B8940F);
      box-shadow:0 8px 24px rgba(201,162,39,.3);
      animation:lp-btn 7s ease-in-out infinite;
      transition:transform .18s;
    }
    .lp-btn::before { content:''; position:absolute; top:0; left:0; right:0; height:48%; background:linear-gradient(180deg,rgba(255,255,255,.3),transparent); border-radius:50px 50px 0 0; pointer-events:none; }
    .lp-btn::after  { content:''; position:absolute; top:-10%; left:-35%; width:28%; height:120%; background:linear-gradient(90deg,transparent,rgba(255,255,255,.35),transparent); transform:skewX(-18deg); animation:lp-swp 7s ease-in-out infinite 1.5s; }
    .lp-btn:hover  { transform:translateY(-2px); }
    .lp-btn:active { transform:translateY(1px) scale(.98); }

    /* features */
    .lp-feat { margin-top:1.7rem; width:100%; }
    .lp-feat-t { font-size:10.5px; font-weight:700; color:#C9A227; letter-spacing:.07em; text-transform:uppercase; margin-bottom:.9rem; }
    .lp-feat-g { display:grid; grid-template-columns:repeat(4,1fr); gap:6px; }
    .lp-fi { display:flex; flex-direction:column; align-items:center; gap:6px; }
    .lp-fi-ico { width:40px; height:40px; border-radius:50%; border:1.5px solid rgba(201,162,39,.28); background:rgba(255,255,255,.6); display:flex; align-items:center; justify-content:center; }
    .lp-fi-lbl { font-size:9px; color:#555; font-weight:600; text-align:center; line-height:1.4; }
    ...
    .qx-slide.qx-sl-has-img::before { display: none; }
    ...
    /* Full-bleed background image for DB banners (Content > Banners) */
    .qx-sl-bgimg {
    ...
    .qx-sl-bgoverlay {
      ...
      background: linear-gradient(90deg, rgba(0,0,0,.72) 0%, rgba(0,0,0,.42) 50%, rgba(0,0,0,.15) 100%);
    }
    ...
    /* Named theme aliases used by admin-managed announcement slides (slide_theme field) */
    .qx-sl-theme-gold   { background: linear-gradient(135deg, #0d0d0d 0%, #1a1200 50%, #111 100%); }
    .qx-sl-theme-blue   { background: linear-gradient(135deg, #0a1628 0%, #0d2044 50%, #091430 100%); }
    .qx-sl-theme-green  { background: linear-gradient(135deg, #051a10 0%, #08280f 50%, #041410 100%); }
    .qx-sl-theme-purple { background: linear-gradient(135deg, #12082a 0%, #1e0e42 50%, #0e0620 100%); }
    .qx-sl-theme-indigo { background: linear-gradient(135deg, #0f0a1a 0%, #1a1030 50%, #100828 100%); }
    .qx-sl-theme-red    { background: linear-gradient(135deg, #1a0505 0%, #2a0a0a 50%, #1a0808 100%); }
    .qx-sl-theme-gold .qx-sl-glow   { background: radial-gradient(circle, rgba(201,162,39,.22) 0%, transparent 70%); }
    .qx-sl-theme-blue .qx-sl-glow   { background: radial-gradient(circle, rgba(59,130,246,.22) 0%, transparent 70%); }
    .qx-sl-theme-green .qx-sl-glow  { background: radial-gradient(circle, rgba(34,197,94,.20) 0%, transparent 70%); }
    .qx-sl-theme-purple .qx-sl-glow { background: radial-gradient(circle, rgba(139,92,246,.22) 0%, transparent 70%); }
    .qx-sl-theme-indigo .qx-sl-glow { background: radial-gradient(circle, rgba(99,102,241,.22) 0%, transparent 70%); }
    .qx-sl-theme-red .qx-sl-glow    { background: radial-gradient(circle, rgba(239,68,68,.24) 0%, transparent 70%); }
    .qx-sl-theme-gold .qx-sl-bot-line   { background: linear-gradient(90deg,transparent,rgba(201,162,39,.6),transparent); }
    .qx-sl-theme-blue .qx-sl-bot-line   { background: linear-gradient(90deg,transparent,rgba(59,130,246,.5),transparent); }
    .qx-sl-theme-green .qx-sl-bot-line  { background: linear-gradient(90deg,transparent,rgba(34,197,94,.5),transparent); }
    .qx-sl-theme-purple .qx-sl-bot-line { background: linear-gradient(90deg,transparent,rgba(139,92,246,.5),transparent); }
    .qx-sl-theme-indigo .qx-sl-bot-line { background: linear-gradient(90deg,transparent,rgba(99,102,241,.5),transparent); }
    .qx-sl-theme-red .qx-sl-bot-line    { background: linear-gradient(90deg,transparent,rgba(239,68,68,.6),transparent); }
    ...
    * { -webkit-tap-highlight-color: transparent; }
    ...
    .chat-hdr {
      display:flex; align-items:center; gap:12px;
      padding:18px 20px 14px; border-bottom:1px solid #F0EDE7;
      flex-shrink:0;
    }
    .chat-hdr-av {
    ...
    .chat-hdr-name { font-size:15px; font-weight:800; color:#111; }
    .chat-hdr-status { font-size:11px; color:#22c55e; font-weight:600; display:flex; align-items:center; gap:5px; }
    .chat-hdr-dot { width:7px; height:7px; border-radius:50%; background:#22c55e; display:inline-block; }
    .chat-close-btn {
      width:34px; height:34px; border-radius:50%; border:none;
      background:#F5F3EE; cursor:pointer; font-size:18px; color:#666;
      display:flex; align-items:center; justify-content:center; flex-shrink:0;
      transition:background .18s;
    }
    .chat-close-btn:hover { background:#EDE9E0; }
    /* messages area */
    .chat-msgs {
    ...
    .chat-bubble-wrap.admin { align-items:flex-start; }
    .chat-bubble {
    ...
    .chat-time {
      font-size:9px; color:#bbb; margin-top:3px; padding:0 4px;
    }
    /* empty state */
    .chat-empty {
    ...
    .chat-empty-title { font-size:15px; font-weight:700; color:#1A1A1A; }
    .chat-empty-sub   { font-size:12px; color:#999; line-height:1.6; }
    /* typing indicator */
    .chat-typing {
      display:none; align-items:center; gap:4px; padding:0 16px 8px;
    }
    ...
    .chat-input-row {
      ...
      padding:12px 16px 20px; border-top:1px solid #F0EDE7; flex-shrink:0;
    }
    .chat-textarea {
      flex:1; min-height:42px; max-height:120px;
      border:1.5px solid #E5E2DC; border-radius:14px;
      padding:10px 14px; font-size:13px; font-family:inherit;
      resize:none; outline:none; line-height:1.5;
      transition:border-color .18s;
      background:#FAFAF8; color:#1A1A1A;
    }
    .chat-textarea:focus { border-color:rgba(212,175,55,.6); background:#fff; }
    .chat-send-btn {
      width:42px; height:42px; border-radius:14px; flex-shrink:0;
      background:linear-gradient(135deg,#E8C96B,#D4AF37);
      border:none; cursor:pointer; color:#111; font-size:18px;
      display:flex; align-items:center; justify-content:center;
      box-shadow:0 4px 14px rgba(212,175,55,.35);
      transition:all .2s; flex-shrink:0;
    }
    .chat-send-btn:hover { transform:scale(1.07); box-shadow:0 6px 18px rgba(212,175,55,.45); }
...
<div class="page active hd" id="page-home">
  ...
  <div class="qx-qa-wrap">
    ...
    <!-- 9. Announcements -->
    <button class="qx-qa-item" onclick="openAnnouncementsListModal()">
      <div class="qx-qa-circle qx-qa-gold">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#9B7A10" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 11l18-5v12L3 14v-3z"/><path d="M11.6 16.8a2 2 0 0 1-3.2 2.4L7 17"/></svg>
      </div>
      <span class="qx-qa-lbl">Announcements</span>
    </button>
      ...
      <div class="wallet-networks">
        <div class="wallet-net-card">
          ...
          <div>
            ...
            <div class="wallet-net-sub">BNB Smart Chain — Fast & low fees</div>
          </div>
          <span class="wallet-net-badge wnet-active">Supported</span>
        </div>
      </div>
...
<div class="page" id="page-about">
  <div class="about-page">
    ...
    <!-- ─── S1b: NEWS ──────────────────────────────── -->
    <div class="ab-section" id="ab-news-section" style="display:none;">
      <div class="ab-section-tag">LATEST UPDATES</div>
      <div class="ab-section-h">News & <em>Announcements</em></div>
      <div class="ab-faq-list" id="ab-news-list"></div>
    </div>
    ...
    <div class="ab-section">
      ...
      <div class="ab-net-grid">
        <div class="ab-net-card">
          <div class="ab-net-logo ab-net-logo-bnb">
            <svg width="28" height="28" viewBox="0 0 32 32" fill="none">
              <path d="M16 4L20.2 8.2L16 12.4L11.8 8.2L16 4Z" fill="#F0B90B"/>
              <path d="M7.6 12.4L11.8 16.6L7.6 20.8L3.4 16.6L7.6 12.4Z" fill="#F0B90B"/>
              <path d="M24.4 12.4L28.6 16.6L24.4 20.8L20.2 16.6L24.4 12.4Z" fill="#F0B90B"/>
              <path d="M16 20.8L20.2 25L16 29.2L11.8 25L16 20.8Z" fill="#F0B90B"/>
              <path d="M16 13.3L18.7 16L16 18.7L13.3 16L16 13.3Z" fill="#F0B90B"/>
            </svg>
          </div>
          <div class="ab-net-info">
            <div class="ab-net-name">BEP20</div>
            <div class="ab-net-sub">USDT — BNB Smart Chain</div>
          </div>
          <div class="ab-net-desc">Fast confirmations and low network fees — the recommended network for all deposits and withdrawals on QAVIX GLOBAL.</div>
    ...
    <div class="ab-section">
      ...
      <div class="ab-faq-list" id="ab-faq-list">
    ...
    <div class="ab-footer">
      ...
      <div class="ab-footer-cols">
        ...
        <div>
          ...
          <div class="ab-footer-link" onclick="openLegalPage('privacy')">Privacy Policy</div>
          <div class="ab-footer-link" onclick="openLegalPage('terms')">Terms of Service</div>
  ...
  <div class="pf-body">
    ...
    <div class="pf-group">
      ...
      <div class="pf-group-cards">
        ...
        <button class="pf-fcard" onclick="openLegalPage('terms')">
          <div class="pf-fcard-icon" style="background:linear-gradient(135deg,#F8F6F2,#F1EEE8);border:1px solid rgba(17,17,17,.1)">📄</div>
          <div class="pf-fcard-body">
        ...
        <button class="pf-fcard" onclick="openLegalPage('privacy')">
          <div class="pf-fcard-icon" style="background:linear-gradient(135deg,#F8F6F2,#F1EEE8);border:1px solid rgba(17,17,17,.1)">🔏</div>
          <div class="pf-fcard-body">
...
<div class="lp-wrap" id="lp-wrap">
  ...
  <!-- concentric gold rings + glow -->
  <div class="lp-cg"></div>
  <div class="lp-rg lp-rg1"></div>
  <div class="lp-rg lp-rg2"></div>
  <div class="lp-rg lp-rg3"></div>

  <!-- tiny drifting particles -->
  <div class="lp-pts" id="lp-pts"></div>
  ...
  <div class="lp-box">
    ...
    <div class="lp-brand"><span class="q">QAVIX </span><span class="g">GLOBAL</span></div>
    <div class="lp-tag">Premium Investment Platform</div>

    <div class="lp-div"><div class="lp-div-l"></div><div class="lp-div-d"></div><div class="lp-div-l"></div></div>

    <button class="lp-btn" onclick="showPage('login')">
    ...
    <div class="lp-h1">Invest Smart, Earn <em>Daily</em></div>
    <div class="lp-p">Secure crypto investment platform<br>trusted by thousands worldwide.</div>

    <div class="lp-feat">
      <div class="lp-feat-t">Your Growth, Our Priority</div>
      <div class="lp-feat-g">
        <div class="lp-fi">
          <div class="lp-fi-ico"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C9A227" stroke-width="1.8" stroke-linecap="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div>
          <div class="lp-fi-lbl">Bank Level Security</div>
        </div>
        <div class="lp-fi">
          <div class="lp-fi-ico"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C9A227" stroke-width="1.8" stroke-linecap="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13.5 19.79 19.79 0 0 1 1.6 4.87 2 2 0 0 1 3.58 2.68h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 10.1a16 16 0 0 0 6 6l.92-1.88a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 21.5 16.5"/></svg></div>
          <div class="lp-fi-lbl">24/7 Support</div>
        </div>
        <div class="lp-fi">
          <div class="lp-fi-ico"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C9A227" stroke-width="1.8" stroke-linecap="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg></div>
          <div class="lp-fi-lbl">Fast Withdraw</div>
        </div>
        <div class="lp-fi">
          <div class="lp-fi-ico"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#C9A227" stroke-width="1.8" stroke-linecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg></div>
          <div class="lp-fi-lbl">Transparent System</div>
        </div>
      </div>
    </div>
  ...
  <script>
  (function(){
    var c=document.getElementById('lp-pts');if(!c)return;
    [{s:3,x:14,y:18,tx:18,ty:-30,d:22,dl:0},{s:2,x:84,y:14,tx:-14,ty:-26,d:26,dl:3},
     {s:3,x:20,y:72,tx:20,ty:-28,d:24,dl:6},{s:2,x:78,y:58,tx:-16,ty:-32,d:20,dl:2},
     {s:2,x:10,y:46,tx:24,ty:-22,d:28,dl:8},{s:3,x:90,y:36,tx:-22,ty:-30,d:18,dl:4},
     {s:2,x:48,y:10,tx:10,ty:-24,d:25,dl:5},{s:2,x:55,y:86,tx:-12,ty:-20,d:23,dl:7}
    ].forEach(function(p){
      var e=document.createElement('div');e.className='lp-pt';
      e.style.cssText='width:'+p.s+'px;height:'+p.s+'px;left:'+p.x+'%;top:'+p.y+'%;--tx:'+p.tx+'px;--ty:'+p.ty+'px;--d:'+p.d+'s;--dl:'+p.dl+'s';
      c.appendChild(e);
    });
  })();
  </script>
        ...
        <form class="glass-form" onsubmit="doRegister(event)">
          ...
          <label style="display:flex;align-items:flex-start;gap:10px;cursor:pointer">
            ...
            <span style="font-size:12px;color:var(--muted);line-height:1.6">
              I agree to the
              <button type="button" onclick="openLegalPage('terms')" style="background:none;border:none;color:var(--gold);font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;padding:0">Terms & Conditions</button>
              and
              <button type="button" onclick="openLegalPage('privacy')" style="background:none;border:none;color:var(--gold);font-size:12px;font-weight:700;cursor:pointer;font-family:inherit;padding:0">Privacy Policy</button>
            </span>
          </label>
...
<script>
  ...
  function showPage(name) {
    ...
    if(name==='about') { setTimeout(loadFaqFromServer, 100); setTimeout(loadNewsFromServer, 120); }
  ...
  function updateInvCalc() {
    ...
    var feePercent = (_paymentInfo && _paymentInfo.withdrawalFeePercent) || 5;
    var fee   = total * (feePercent / 100);
    var net   = total - fee;
    // Update the fee label text with live %
    var feeLabel = document.querySelector('#inv-calc-fee');
    var feeLabelParent = feeLabel && feeLabel.previousElementSibling;
    if (feeLabelParent) feeLabelParent.textContent = 'Withdrawal Fee (' + feePercent + '%)';
  ...
  // ══ NOTIFICATIONS ═════════════════════════════════════
  var _notifCache = [];
  const notifIcons = {profit:'💰',deposit:'⬇️',withdrawal:'💸',commission:'🤝',bonus:'🎁',security:'🔒',system:'🔔',announcement:'📢',default:'🔔'};
  const timeAgo = (d) => {
  ...
  function renderNotifications() {
    ...
    list.innerHTML = _notifCache.slice(0,15).map(n=>`
      <div class="hdr-notif-row ${n.read?'':'unread'}" onclick="handleNotifClick('${n.id}',this)">
  ...
  function handleNotifClick(id, el) {
    ...
    if (n && n.type === 'announcement' && typeof showAnnouncementDetail === 'function') {
      showAnnouncementDetail(n.id, n.title, n.body);
    }
  }
    ...
    overlay.innerHTML = `
      <div class="qx-modal" style="max-width:420px">
        ...
        <div class="qx-modal-head">
          ...
          <div style="display:flex;align-items:center;gap:10px">
            ${hasPass ? `<button onclick="openForgotWdPassFlow()" style="background:none;border:none;cursor:pointer;font-size:11px;font-weight:700;color:var(--gold);font-family:inherit;white-space:nowrap;padding:0">Forgot Password?</button>` : ''}
            <button class="qx-modal-close" onclick="document.getElementById('qx-wdpass-modal').remove()">✕</button>
          </div>
        </div>
        <div style="padding:0 1.2rem 1.4rem;display:flex;flex-direction:column;gap:10px">
          <div style="font-size:12px;color:var(--muted);line-height:1.6">
            ${hasPass ? 'Enter your old withdrawal password and new one. No OTP required if old password is correct.' : 'Set a 6-digit PIN or password for withdrawals.'}
          </div>
          ...
          <button id="wdp-btn" onclick="submitWithdrawalPass(${hasPass?true:false})"
  ...
  async function submitWithdrawalPass(isChange) {
    ...
    var btn  = document.getElementById('wdp-btn');
    ...
    try {
      if (isChange) {
        // Old pass present → change directly, no OTP
        if (!oldP) { if(err){err.textContent='Please enter your old password';err.style.display='block';} if(btn){btn.disabled=false;btn.textContent='Change Password';} return; }
      ...
      if(btn){btn.disabled=false;btn.textContent=isChange?'Change Password':'Set Password';}
    }
  }

  // -- Withdrawal Password Forgot Flow --
  function openForgotWdPassFlow() {
    document.getElementById('qx-wdpass-modal')?.remove();
    showToast('Sending OTP to your email...','info');
    API.req('POST', '/user/change-withdrawal-password', { forgotMode: true }).then(function(r) {
      if (!r || !r.requireOtp) { showToast('Failed to send OTP','error'); return; }
      showOTPModal({
        title: 'Forgot Withdrawal Password',
        subtitle: 'OTP sent to your registered email',
        onVerify: function(otp) {
          document.getElementById('qx-otp-modal')?.remove();
          openSetNewWdPassModal(otp);
        },
        onResend: function() {
          return API.req('POST', '/user/change-withdrawal-password', { forgotMode: true });
        }
      });
    }).catch(function(e){ showToast(e.message||'Failed to send OTP','error'); });
  }

  function openSetNewWdPassModal(verifiedOtp) {
    var overlay = document.createElement('div');
    overlay.className = 'qx-overlay';
    overlay.id = 'qx-setnewwdpass-modal';
    overlay.dataset.otp = verifiedOtp;
    overlay.innerHTML = `
      <div class="qx-modal" style="max-width:420px">
        <div class="qx-modal-handle"></div>
        <div class="qx-modal-head">
          <div style="display:flex;align-items:center;gap:10px">
            <div class="qx-modal-icon-wrap" style="background:rgba(201,162,39,.12)">🔑</div>
            <div class="qx-modal-title">Set New Withdrawal Password</div>
          </div>
          <button class="qx-modal-close" onclick="document.getElementById('qx-setnewwdpass-modal').remove()">✕</button>
        </div>
        <div style="padding:0 1.2rem 1.4rem;display:flex;flex-direction:column;gap:10px">
          <div style="font-size:12px;color:var(--muted);line-height:1.6">OTP verified — Enter your new withdrawal password.</div>
          <input id="snwp-new" type="password" placeholder="New withdrawal password (min 6)"
            ...
            onfocus="this.style.borderColor='var(--gold)'" onblur="this.style.borderColor='var(--border)'"/>
          <input id="snwp-confirm" type="password" placeholder="Confirm new password"
            style="width:100%;padding:12px 14px;border-radius:11px;border:1.5px solid var(--border);
            background:var(--bg);color:var(--black);font-size:14px;font-family:inherit;outline:none;
            transition:border-color .2s;box-sizing:border-box"
            onfocus="this.style.borderColor='var(--gold)'" onblur="this.style.borderColor='var(--border)'"/>
          <div id="snwp-err" style="font-size:12px;color:#dc2626;display:none"></div>
          <button id="snwp-btn" onclick="submitForgotWdPass()"
            ...
            cursor:pointer;font-family:inherit;transition:all .2s">Change Password</button>
        </div>
      </div>`;
    document.body.appendChild(overlay);
    overlay.addEventListener('click', e=>{ if(e.target===overlay) overlay.remove(); });
  ...
  async function submitForgotWdPass() {
    var modal = document.getElementById('qx-setnewwdpass-modal');
    var otp  = modal?.dataset?.otp;
    var np   = document.getElementById('snwp-new')?.value;
    var nc   = document.getElementById('snwp-confirm')?.value;
    var er   = document.getElementById('snwp-err');
    var btn  = document.getElementById('snwp-btn');
    if (!np || np.length < 6) { if(er){er.textContent='Min 6 characters';er.style.display='block';} return; }
    if (np !== nc) { if(er){er.textContent='Passwords do not match';er.style.display='block';} return; }
    if(er) er.style.display='none';
    if(btn){ btn.disabled=true; btn.textContent='Changing...'; }
    try {
      await API.req('POST', '/user/change-withdrawal-password', { newPassword: np, otp });
      document.getElementById('qx-setnewwdpass-modal')?.remove();
      showToast('Withdrawal password changed successfully!','success');
    } catch(err) {
      if(er){ er.textContent=err.message||'Failed'; er.style.display='block'; }
  ...
  // Load live payment limits & fee from backend on startup
  var _paymentInfo = { withdrawalFeePercent: 5, withdrawalMin: 10, withdrawalMax: 10000 };
  // Load live fee/limits after page fully ready — use timeout since DOMContentLoaded
  // may have already fired by the time this inline script runs (scripts are in body).
  function _loadPaymentInfo() {
    if (typeof API !== 'undefined' && API.req) {
      API.req('GET', '/content/payment-info').then(function(r){
        if (r && r.data) _paymentInfo = Object.assign({}, _paymentInfo, r.data);
      }).catch(function(){});
    }
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', _loadPaymentInfo);
  } else {
    setTimeout(_loadPaymentInfo, 0);
  }
  ...
  function confirmWithdraw() {
    ...
    var fee = +(parseFloat(amt) * (_paymentInfo.withdrawalFeePercent / 100)).toFixed(2);
    ...
    overlay.innerHTML = `
      <div class="qx-modal" style="max-width:420px">
        ...
        <div class="qx-modal-head">
          ...
          <div style="display:flex;align-items:center;gap:10px">
            <button onclick="openForgotLoginPassFlow()" style="background:none;border:none;cursor:pointer;font-size:11px;font-weight:700;color:var(--gold);font-family:inherit;white-space:nowrap;padding:0">Forgot Password?</button>
            <button class="qx-modal-close" onclick="document.getElementById('qx-chpass-modal').remove()">✕</button>
          </div>
        </div>
        <div style="padding:0 1.2rem 1.4rem;display:flex;flex-direction:column;gap:10px">
          <div style="font-size:12px;color:var(--muted);line-height:1.6">Enter your old password to change it. No OTP required if old password is correct.</div>
          <input id="cp-old" type="password" placeholder="Old Password"
            style="width:100%;padding:12px 14px;border-radius:11px;border:1.5px solid var(--border);
            background:var(--bg);color:var(--black);font-size:14px;font-family:inherit;outline:none;
            transition:border-color .2s;box-sizing:border-box"
            onfocus="this.style.borderColor='var(--gold)'" onblur="this.style.borderColor='var(--border)'"/>
          ...
          <button id="cp-btn" onclick="submitChangePass()" style="width:100%;padding:13px;border-radius:12px;
            background:var(--black);color:var(--gold);border:none;font-size:14px;font-weight:800;
            cursor:pointer;font-family:inherit;transition:all .2s">Change Password</button>
        </div>
      </div>`;
  ...
  async function submitChangePass() {
    ...
    if (!oldP) { if(er){er.textContent='Please enter your old password';er.style.display='block';} return; }
    if (!np || np.length < 8) { if(er){er.textContent='New password: min 8 characters';er.style.display='block';} return; }
    if (np !== nc) { if(er){er.textContent='Passwords do not match';er.style.display='block';} return; }
    if(er) er.style.display='none';
    if(btn){ btn.disabled=true; btn.textContent='Changing...'; }
    try {
      await API.req('POST', '/user/change-password', { oldPassword: oldP, newPassword: np });
  ...
  // -- Login Password Forgot Flow --
  function openForgotLoginPassFlow() {
    document.getElementById('qx-chpass-modal')?.remove();
    showToast('Sending OTP to your email...','info');
    API.req('POST', '/user/change-password', { forgotMode: true }).then(function(r) {
      if (!r || !r.requireOtp) { showToast('Failed to send OTP','error'); return; }
      showOTPModal({
        ...
        onVerify: function(otp) {
          document.getElementById('qx-otp-modal')?.remove();
          openSetNewLoginPassModal(otp);
        },
        onResend: function() {
          return API.req('POST', '/user/change-password', { forgotMode: true });
        }
      });
    }).catch(function(e){ showToast(e.message||'Failed to send OTP','error'); });
  }

  function openSetNewLoginPassModal(verifiedOtp) {
    var overlay = document.createElement('div');
    overlay.className = 'qx-overlay';
    overlay.id = 'qx-setnewpass-modal';
    overlay.innerHTML = `
      <div class="qx-modal" style="max-width:420px">
        <div class="qx-modal-handle"></div>
        <div class="qx-modal-head">
          <div style="display:flex;align-items:center;gap:10px">
            <div class="qx-modal-icon-wrap" style="background:rgba(201,162,39,.12)">🔑</div>
            <div class="qx-modal-title">Set New Password</div>
          </div>
          <button class="qx-modal-close" onclick="document.getElementById('qx-setnewpass-modal').remove()">✕</button>
        </div>
        <div style="padding:0 1.2rem 1.4rem;display:flex;flex-direction:column;gap:10px">
          <div style="font-size:12px;color:var(--muted);line-height:1.6">OTP verified — Enter your new login password.</div>
          <input id="snp-new" type="password" placeholder="New Password (min 8 chars)"
            style="width:100%;padding:12px 14px;border-radius:11px;border:1.5px solid var(--border);
            background:var(--bg);color:var(--black);font-size:14px;font-family:inherit;outline:none;
            transition:border-color .2s;box-sizing:border-box"
            onfocus="this.style.borderColor='var(--gold)'" onblur="this.style.borderColor='var(--border)'"/>
          <input id="snp-confirm" type="password" placeholder="Confirm New Password"
            style="width:100%;padding:12px 14px;border-radius:11px;border:1.5px solid var(--border);
            background:var(--bg);color:var(--black);font-size:14px;font-family:inherit;outline:none;
            transition:border-color .2s;box-sizing:border-box"
            onfocus="this.style.borderColor='var(--gold)'" onblur="this.style.borderColor='var(--border)'"/>
          <div id="snp-err" style="font-size:12px;color:#dc2626;display:none"></div>
          <button id="snp-btn" onclick="submitForgotLoginPass()"
            style="width:100%;padding:13px;border-radius:12px;
            background:var(--black);color:var(--gold);border:none;font-size:14px;font-weight:800;
            cursor:pointer;font-family:inherit;transition:all .2s">Change Password</button>
        </div>
    ...
    document.body.appendChild(overlay);
    // store otp in closure via dataset
    overlay.dataset.otp = verifiedOtp;
    overlay.addEventListener('click', e=>{ if(e.target===overlay) overlay.remove(); });
  }

  async function submitForgotLoginPass(otp) {
    var modal = document.getElementById('qx-setnewpass-modal');
    var resolvedOtp = otp || modal?.dataset?.otp;
    var np  = document.getElementById('snp-new')?.value;
    var nc  = document.getElementById('snp-confirm')?.value;
    var er  = document.getElementById('snp-err');
    var btn = document.getElementById('snp-btn');
    if (!np || np.length < 8) { if(er){er.textContent='Min 8 characters';er.style.display='block';} return; }
    if (np !== nc) { if(er){er.textContent='Passwords do not match';er.style.display='block';} return; }
    if(er) er.style.display='none';
    if(btn){ btn.disabled=true; btn.textContent='Changing...'; }
    try {
      await API.req('POST', '/user/change-password', { newPassword: np, otp: resolvedOtp });
      document.getElementById('qx-setnewpass-modal')?.remove();
      showToast('Password changed successfully!','success');
    } catch(err) {
      if(er){ er.textContent=err.message||'Failed'; er.style.display='block'; }
      if(btn){ btn.disabled=false; btn.textContent='Change Password'; }
  ...
  function confirmDeposit() {
    var u = API.getUser() || {};
    var depositAddr = '0x742d35Cc6634C0532925a3b8D4C9C0a6bEf4267'; // Platform BEP20 deposit address
    openModal({
      icon:'💰', iconBg:'rgba(34,197,94,.1)',
      title:'Deposit USDT',
      body:'Send USDT (BEP20) to the platform address below. Your balance will be credited after manual verification by admin.',
      rows:[
        {l:'Network',      v:'BEP20 (BNB Smart Chain)'},
        {l:'Min Deposit',  v:'$10 USDT'},
        {l:'Confirm Time', v:'Within 24 hours'},
        {l:'Address',      v:depositAddr, cls:'gold'}
      ],
...
(function() {
  var STATIC_COUNT = 6; // the 6 hardcoded slides — never fewer than these
  var TOTAL    = STATIC_COUNT;
  ...
  // Colour themes for DB banners — cycles through 6 slide themes
  var SLIDE_THEMES = ['qx-sl-1','qx-sl-2','qx-sl-3','qx-sl-4','qx-sl-5','qx-sl-6'];
  var TAG_THEMES   = ['qx-sl-tag-gold','qx-sl-tag-green','qx-sl-tag-blue','qx-sl-tag-red','qx-sl-tag-green','qx-sl-tag-indigo'];
  var CTA_THEMES   = ['qx-sl-cta-gold','qx-sl-cta-green','qx-sl-cta-blue','qx-sl-cta-red','qx-sl-cta-green','qx-sl-cta-indigo'];

  async function injectDBBanners(){
    try {
      var r = await API.req('GET', '/content/banners');
      if (!r || !r.data || !r.data.banners || !r.data.banners.length) return;
      var track = document.getElementById('qx-slider-track');
      var dotsWrap = document.getElementById('qx-slider-dots');
      if (!track) return;
      r.data.banners.forEach(function(b, i){
        var theme = SLIDE_THEMES[i % SLIDE_THEMES.length];
        var tagClass = TAG_THEMES[i % TAG_THEMES.length];
        var ctaClass = CTA_THEMES[i % CTA_THEMES.length];
        var idx = TOTAL;
        // Build slide using the exact same structure as static slides
        var slide = document.createElement('div');
        slide.className = 'qx-slide ' + theme + (b.imageUrl ? ' qx-sl-has-img' : '');
        slide.dataset.idx = idx;
        slide.innerHTML =
          (b.imageUrl ? '<img class="qx-sl-bgimg" src="'+escapeHtml(b.imageUrl)+'" onerror="this.remove()"/><div class="qx-sl-bgoverlay"></div>' : '') +
          '<div class="qx-sl-glow"></div>' +
          '<div class="qx-sl-bot-line"></div>' +
          (b.imageUrl ? '' : '<div class="qx-sl-illu">📢</div>') +
          '<div class="qx-sl-top">' +
            '<span class="qx-sl-tag '+tagClass+'">📢 Announcement</span>' +
            '<span class="qx-sl-num">0' + (idx+1) + ' / ' + (TOTAL+r.data.banners.length) + '</span>' +
          '</div>' +
          '<div class="qx-sl-body">' +
            '<div class="qx-sl-headline">' + escapeHtml(b.title) + (b.subtitle ? '<br><em>'+escapeHtml(b.subtitle)+'</em>' : '') + '</div>' +
          '</div>' +
          (b.linkUrl
            ? '<div class="qx-sl-bottom"><button class="qx-sl-cta '+ctaClass+'" data-link="'+escapeHtml(b.linkUrl)+'">Learn More <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></button></div>' : '');
        track.appendChild(slide);
        if (b.linkUrl){
          var ctaBtn = slide.querySelector('.qx-sl-cta');
          if (ctaBtn) ctaBtn.addEventListener('click', function(){ window.open(this.getAttribute('data-link'), '_blank'); });
        }
        // Add a dot
        if (dotsWrap){
          var dot = document.createElement('div');
          dot.className = 'qx-sl-dot';
          dot.setAttribute('onclick', 'qxGoTo('+idx+')');
          dotsWrap.appendChild(dot);
        }
        TOTAL++;
      });
      // Update the static slides' N/TOTAL counters
      document.querySelectorAll('.qx-sl-num').forEach(function(el, i){
        var parts = el.textContent.split('/');
        if(parts.length===2) el.textContent = parts[0].trim() + ' / ' + TOTAL;
      });
    } catch(e) { /* fail silently — static slides still work */ }
  }

  async function injectAnnouncements(){
    try {
      var r = await API.req('GET', '/announcements/active');
      if (!r || !r.data || !r.data.announcements) return;
      var banners = r.data.announcements.filter(function(a){ return a.type === 'banner'; });
      if (!banners.length) return;
      var track = document.getElementById('qx-slider-track');
      var dotsWrap = document.getElementById('qx-slider-dots');
      if (!track) return;
      banners.forEach(function(a, i){
        var themeName = a.slideTheme || 'gold';
        var bgClass = 'qx-sl-theme-' + themeName;
        var tagClass = 'qx-sl-tag-' + themeName;
        var ctaClass = 'qx-sl-cta-' + themeName;
        var idx = TOTAL;
        var shortMsg = (a.message||'').length > 90 ? a.message.slice(0,90)+'…' : (a.message||'');
        var slide = document.createElement('div');
        slide.className = 'qx-slide ' + bgClass;
        slide.dataset.idx = idx;
        ...
        slide.innerHTML =
          ...
          '<div class="qx-sl-top">' +
            '<span class="qx-sl-tag '+tagClass+'">📢 Announcement</span>' +
            '<span class="qx-sl-num">0' + (idx+1) + ' / ' + (TOTAL+banners.length) + '</span>' +
          '</div>' +
          '<div class="qx-sl-body">' +
            '<div class="qx-sl-headline">' + escapeHtml(a.title) + '</div>' +
            '<div class="qx-sl-sub">' + escapeHtml(shortMsg) + '</div>' +
          '</div>' +
          '<div class="qx-sl-bottom"><button class="qx-sl-cta '+ctaClass+'" type="button">View Details ' +
            '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg></button></div>';
        slide.addEventListener('click', function(){ if (typeof showAnnouncementDetail === 'function') showAnnouncementDetail(a); });
        track.appendChild(slide);
        if (dotsWrap){
          var dot = document.createElement('div');
          dot.className = 'qx-sl-dot';
      ...
      document.querySelectorAll('.qx-sl-num').forEach(function(el){
        var parts = el.textContent.split('/');
        if(parts.length===2) el.textContent = parts[0].trim() + ' / ' + TOTAL;
      });
    } catch(e) { /* fail silently — static slides still work */ }
  ...
  function init() {
    ...
    injectDBBanners().then(injectAnnouncements).finally(function(){ startTimer(); });
  }
...
// Pull FAQ entries from the admin-managed content API. If it fails or returns
// nothing, the original hardcoded FAQ markup (already in the page) is left untouched.
var _faqLoaded = false;
async function loadFaqFromServer() {
  if (_faqLoaded) return;
  try {
    var r = await API.req('GET', '/content/faq');
    if (!r || !r.data || !r.data.faqs || r.data.faqs.length === 0) return;
    var list = document.getElementById('ab-faq-list');
    if (!list) return;
    list.innerHTML = r.data.faqs.map(function(f){
      return '<div class="ab-faq-item">' +
        '<button class="ab-faq-q" onclick="abFaq(this)">' +
          '<span class="ab-faq-q-text">' + escapeHtml(f.question) + '</span>' +
          '<div class="ab-faq-chevron">▾</div>' +
        '</button>' +
        '<div class="ab-faq-answer"><div class="ab-faq-answer-inner">' + escapeHtml(f.answer) + '</div></div>' +
      '</div>';
    }).join('');
    _faqLoaded = true;
  } catch(e) { /* keep the static fallback FAQ already in the markup */ }
}
function escapeHtml(s){
  var d = document.createElement('div'); d.textContent = s == null ? '' : String(s); return d.innerHTML;
}

// Pull recent published news posts. Section stays hidden if there's nothing to show,
// so it never displays an awkward empty block for platforms with no posts yet.
var _newsLoaded = false;
async function loadNewsFromServer() {
  if (_newsLoaded) return;
  try {
    var r = await API.req('GET', '/content/news?limit=10');
    if (!r || !r.data || !r.data.news || r.data.news.length === 0) return;
    var section = document.getElementById('ab-news-section');
    var list = document.getElementById('ab-news-list');
    if (!list || !section) return;
    list.innerHTML = r.data.news.map(function(n){
      var d = new Date(n.publishedAt);
      var dateStr = d.toLocaleDateString('en-US', {month:'short', day:'numeric', year:'numeric'});
      var bodyHtml = escapeHtml(n.body || n.excerpt || '').replace(/\n/g, '<br>');
      return '<div class="ab-faq-item">' +
        '<button class="ab-faq-q" onclick="abFaq(this)">' +
          '<span class="ab-faq-q-text">' + escapeHtml(n.title) + '</span>' +
          '<div class="ab-faq-chevron">▾</div>' +
        '</button>' +
        (n.excerpt ? '<div style="padding:0 20px 10px;margin-top:-8px;font-size:12.5px;color:var(--muted);">' + escapeHtml(n.excerpt) + ' · <span style="color:var(--muted2);">' + dateStr + '</span></div>' : '') +
        '<div class="ab-faq-answer">' +
          '<div class="ab-faq-answer-inner">' + bodyHtml + '</div>' +
        '</div>' +
      '</div>';
    }).join('');
    section.style.display = '';
    _newsLoaded = true;
  } catch(e) { /* no news section if the request fails — nothing breaks */ }
}

// Shared modal for Terms of Service / Privacy Policy, pulled live from the
// admin-managed content_pages table. Falls back to a friendly message if the
// admin hasn't written that page yet.
async function openLegalPage(slug) {
  var overlay = document.createElement('div');
  overlay.className = 'qx-overlay'; overlay.id = 'qx-legal-modal';
  overlay.innerHTML =
    '<div class="qx-modal" style="max-width:560px; max-height:80vh; display:flex; flex-direction:column;">' +
      '<div class="qx-modal-handle"></div>' +
      '<div class="qx-modal-head">' +
        '<div class="qx-modal-title" id="qx-legal-title">Loading…</div>' +
        '<button class="qx-modal-close" onclick="document.getElementById(\'qx-legal-modal\').remove()">✕</button>' +
      '</div>' +
      '<div class="qx-modal-body" id="qx-legal-body" style="overflow-y:auto; white-space:pre-wrap;">Loading…</div>' +
    '</div>';
  overlay.addEventListener('click', function(e){ if (e.target === overlay) overlay.remove(); });
  document.body.appendChild(overlay);
  try {
    var r = await API.req('GET', '/content/pages/' + slug);
    var titleEl = document.getElementById('qx-legal-title');
    var bodyEl = document.getElementById('qx-legal-body');
    if (titleEl) titleEl.textContent = r.data.page.title;
    if (bodyEl) bodyEl.textContent = r.data.page.body;
  } catch(e) {
    var titleEl2 = document.getElementById('qx-legal-title');
    var bodyEl2 = document.getElementById('qx-legal-body');
    if (titleEl2) titleEl2.textContent = slug === 'terms' ? 'Terms & Conditions' : 'Privacy Policy';
    if (bodyEl2) bodyEl2.textContent = 'This page hasn\'t been published yet. Please check back soon.';
  }
}
...
<div class="chat-overlay" id="chat-overlay" onclick="chatOverlayClick(event)">
  <div class="chat-modal" id="chat-modal">
    <!-- Header -->
    <div class="chat-hdr">
      <div class="chat-hdr-av">🎧</div>
      <div class="chat-hdr-info">
        <div class="chat-hdr-name">QAVIX Support</div>
        <div class="chat-hdr-status">
          <span class="chat-hdr-dot"></span>Online · Avg. reply 2 min
        </div>
      </div>
    ...
    <div class="chat-msgs" id="chat-msgs">
      <div class="chat-empty" id="chat-empty">
        ...
        <div class="chat-empty-title">How can we help?</div>
        <div class="chat-empty-sub">Send us a message and our team<br>will reply as soon as possible.</div>
      </div>
    </div>
    ...
    <div class="chat-typing" id="chat-typing">
      ...
      <span style="font-size:11px;color:#999;margin-left:4px">Support is typing…</span>
    </div>
...
<script>
  ...
  var chatMsgIds        = new Set();
  var chatUnreadTimer   = null;
  var chatLastSeenTime  = null;

  function chatStartUnreadPoll() {
    chatStopUnreadPoll();
    chatUnreadTimer = setInterval(async function() {
      if (chatOpen || !API.Auth.isLoggedIn()) return;
      try {
        var qs = chatLastSeenTime ? '?since=' + encodeURIComponent(chatLastSeenTime) : '';
        var r  = await API.req('GET', '/support/messages' + qs);
        if (!r || !r.data) return;
        var newAdmin = (r.data.messages || []).filter(function(m){ return m.direction === 'admin'; });
        var badge = document.getElementById('hdr-chat-badge');
        if (badge && newAdmin.length > 0) {
          badge.textContent = newAdmin.length > 9 ? '9+' : newAdmin.length;
          badge.style.display = 'flex';
        }
      } catch(e) {}
    }, 15000);
  }
  function chatStopUnreadPoll() {
    if (chatUnreadTimer) { clearInterval(chatUnreadTimer); chatUnreadTimer = null; }
  }
  function chatClearBadge() {
    var badge = document.getElementById('hdr-chat-badge');
    if (badge) badge.style.display = 'none';
    chatLastSeenTime = new Date().toISOString();
  }

  function openLiveChat() {
    if (!API.Auth.isLoggedIn()) { showToast('Please login first','error'); return; }
  ...
  async function chatSend() {
    ...
    // Optimistic bubble (temp ID)
    var tempId = 'tmp-' + Date.now();
    chatAddBubble({ id: tempId, direction:'user', message:msg, createdAt: new Date().toISOString() });

    try {
      await API.req('POST', '/support/send', { message: msg });
      // Remove temp, full reload will show real record
    } catch(e) {
      showToast('Failed to send. Try again.', 'error');
      // Remove the optimistic bubble on failure
      var el = document.getElementById('cbubble-' + tempId);
      if (el) el.remove();
      chatMsgIds.delete(tempId);
    }
    btn.disabled = false;
    input.focus();
  }

  // Full reload from DB — called on open
  async function chatRenderFromDB() {
    try {
      var r = await API.req('GET', '/support/messages');
      if (!r || !r.data) return;
      ...
      emptyDiv.innerHTML = '<div class="chat-empty-icon">💬</div>' +
        '<div class="chat-empty-title">How can we help?</div>' +
        '<div class="chat-empty-sub">Send us a message and our team<br>will reply as soon as possible.</div>';
      ...
      if (msgs.length === 0) {
        ...
        msgs.forEach(function(m){ chatAddBubble(m, true); });
        chatLastTime = msgs[msgs.length-1].createdAt;
      }
      chatScrollBottom();
    } catch(e) {}
  }
  ...
  function chatStartPoll() {
    ...
    chatPollTimer = setInterval(async function() {
      if (!chatOpen) return;
      try {
        var qs = chatLastTime ? '?since=' + encodeURIComponent(chatLastTime) : '';
        var r  = await API.req('GET', '/support/messages' + qs);
        if (r && r.data && r.data.messages && r.data.messages.length) {
          var newMsgs = r.data.messages.filter(function(m){ return !chatMsgIds.has(m.id); });
          newMsgs.forEach(function(m){ chatAddBubble(m); hideChatEmpty(); });
          if (newMsgs.length) {
            chatLastTime = r.data.messages[r.data.messages.length-1].createdAt;
            chatScrollBottom();
          }
        }
      } catch(e) {}
  ...
  function chatAddBubble(m, silent) {
    if (chatMsgIds.has(m.id)) return; // skip duplicates
    chatMsgIds.add(m.id);

    var container = document.getElementById('chat-msgs');

    var wrap = document.createElement('div');
    wrap.className = 'chat-bubble-wrap ' + m.direction;
    wrap.id = 'cbubble-' + m.id;

    var d    = new Date(m.createdAt);
    var tStr = d.getHours().toString().padStart(2,'0') + ':' + d.getMinutes().toString().padStart(2,'0');
  ...
  // ── PHASE 5: ANNOUNCEMENTS (admin-managed banner + popup) ────────────────
  var ANN_STYLE_COLORS = {
    info:    { bg:'#EAF1FC', fg:'#3B6FD1' },
    success: { bg:'#E8F7EF', fg:'#1F9D63' },
    warning: { bg:'#FBF3D9', fg:'#9B7A10' },
    urgent:  { bg:'#FBE9E9', fg:'#D14B4B' },
  };
  var QX_ANN_CACHE = {}; // id -> announcement object, populated whenever we fetch
  ...
  async function loadAnnouncements() {
    try {
      ...
      list.forEach(function(a){ QX_ANN_CACHE[a.id] = a; });
      var popups  = list.filter(function(a){ return a.type === 'popup'; });

      // Show at most one popup per browser session, and only if not dismissed already
      var dismissed = JSON.parse(sessionStorage.getItem('qvx_ann_dismissed') || '[]');
      var toShow = popups.find(function(p){ return dismissed.indexOf(p.id) === -1; });
      if (toShow) showAnnouncementPopup(toShow);
    } catch (e) { /* silent — announcements are non-critical */ }
  }

  function showAnnouncementPopup(a) {
    var c = ANN_STYLE_COLORS[a.style] || ANN_STYLE_COLORS.info;
    var overlay = document.createElement('div');
    overlay.className = 'qx-overlay'; overlay.id = 'qx-ann-popup';
    overlay.innerHTML =
      '<div class="qx-modal" style="max-width:420px">' +
        '<div class="qx-modal-handle"></div>' +
        '<div class="qx-modal-head">' +
          '<div style="display:flex;align-items:center;gap:10px">' +
            '<div class="qx-modal-icon-wrap" style="background:' + c.bg + '; color:' + c.fg + '; font-size:20px">📢</div>' +
            '<div class="qx-modal-title">' + escHtml(a.title) + '</div>' +
          '</div>' +
          '<button class="qx-modal-close" onclick="dismissAnnouncementPopup(\'' + a.id + '\')">✕</button>' +
        '</div>' +
        '<div class="qx-modal-body">' + escHtml(a.message) + '</div>' +
        '<div style="padding:.2rem 1.2rem 1.2rem">' +
          '<button onclick="dismissAnnouncementPopup(\'' + a.id + '\')" ' +
          'style="width:100%;padding:11px;border-radius:10px;border:none;background:var(--gold);color:#1a1408;font-weight:800;font-size:13px;cursor:pointer;">Got it</button>' +
        '</div>' +
      '</div>';
    overlay.addEventListener('click', function(e){ if (e.target === overlay) dismissAnnouncementPopup(a.id); });
    document.body.appendChild(overlay);
  }
  ...
  function dismissAnnouncementPopup(id) {
    ...
    sessionStorage.setItem('qvx_ann_dismissed', JSON.stringify(dismissed));
    var el = document.getElementById('qx-ann-popup');
    if (el) el.remove();
  }

  // Detail modal — used by slider slides, the "All Announcements" list, and notification taps.
  // Accepts either a full announcement object, or just an id (looked up from cache / notif fallback).
  function showAnnouncementDetail(aOrId, fallbackTitle, fallbackBody) {
    var a = (typeof aOrId === 'object') ? aOrId : QX_ANN_CACHE[aOrId];
    if (!a) {
      if (!fallbackTitle) return;
      a = { title: fallbackTitle, message: fallbackBody || '', style: 'info' };
    }
    closeAnnouncementDetail();
    var c = ANN_STYLE_COLORS[a.style] || ANN_STYLE_COLORS.info;
    var overlay = document.createElement('div');
    overlay.className = 'qx-overlay'; overlay.id = 'qx-ann-detail';
    overlay.innerHTML =
      '<div class="qx-modal" style="max-width:420px">' +
        '<div class="qx-modal-handle"></div>' +
        '<div class="qx-modal-head">' +
          '<div style="display:flex;align-items:center;gap:10px">' +
            ...
            '<div class="qx-modal-title">' + escHtml(a.title) + '</div>' +
          '</div>' +
          '<button class="qx-modal-close" onclick="closeAnnouncementDetail()">✕</button>' +
        '</div>' +
        '<div class="qx-modal-body">' + escHtml(a.message||'') + '</div>' +
        '<div style="padding:.2rem 1.2rem 1.2rem">' +
          '<button onclick="closeAnnouncementDetail()" ' +
          'style="width:100%;padding:11px;border-radius:10px;border:none;background:var(--gold);color:#1a1408;font-weight:800;font-size:13px;cursor:pointer;">Close</button>' +
        '</div>' +
      '</div>';
    overlay.addEventListener('click', function(e){ if (e.target === overlay) closeAnnouncementDetail(); });
    document.body.appendChild(overlay);
  }
  function closeAnnouncementDetail() {
    var el = document.getElementById('qx-ann-detail');
    if (el) el.remove();
  }

  // "All Announcements" list — opened from the Quick Actions tile
  async function openAnnouncementsListModal() {
    var overlay = document.createElement('div');
    overlay.className = 'qx-overlay'; overlay.id = 'qx-ann-list';
    overlay.innerHTML =
      '<div class="qx-modal" style="max-width:440px">' +
        '<div class="qx-modal-handle"></div>' +
        '<div class="qx-modal-head">' +
          '<div class="qx-modal-title">📢 Announcements</div>' +
          '<button class="qx-modal-close" onclick="document.getElementById(\'qx-ann-list\').remove()">✕</button>' +
        '</div>' +
        '<div class="qx-modal-body" id="qx-ann-list-body" style="max-height:60vh;overflow-y:auto;">' +
          '<div style="text-align:center;padding:24px;color:#999;font-size:12px">Loading…</div>' +
        '</div>' +
      '</div>';
    overlay.addEventListener('click', function(e){ if (e.target === overlay) overlay.remove(); });
    ...
    try {
      var r = await API.req('GET', '/announcements/active');
      var list = (r && r.data && r.data.announcements) || [];
      list.forEach(function(a){ QX_ANN_CACHE[a.id] = a; });
      var body = document.getElementById('qx-ann-list-body');
      if (!body) return;
      if (!list.length) {
        body.innerHTML = '<div style="text-align:center;padding:24px;color:#999;font-size:12px">🔔<br>No announcements right now</div>';
        return;
      }
      body.innerHTML = '';
      list.forEach(function(a){
        ...
        row.style.cssText = 'display:flex;align-items:flex-start;gap:10px;padding:11px 2px;border-bottom:1px solid #f0ede3;cursor:pointer;';
        row.innerHTML =
          '<div style="width:32px;height:32px;flex-shrink:0;border-radius:9px;display:flex;align-items:center;justify-content:center;background:' + c.bg + ';color:' + c.fg + ';font-size:15px;">📢</div>' +
          '<div style="flex:1;min-width:0">' +
            '<div style="font-size:13px;font-weight:700;color:#111;">' + escHtml(a.title) + '</div>' +
            '<div style="font-size:11.5px;color:#888;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + escHtml(a.message||'') + '</div>' +
          '</div>';
        row.addEventListener('click', function(){ showAnnouncementDetail(a); });
        body.appendChild(row);
      });
    } catch (e) {
      var body2 = document.getElementById('qx-ann-list-body');
      if (body2) body2.innerHTML = '<div style="text-align:center;padding:24px;color:#999;font-size:12px">Could not load announcements</div>';
    }
  }

  document.addEventListener('DOMContentLoaded', function(){ setTimeout(loadAnnouncements, 600); });


  // ── Branding: apply primaryColor from admin settings ─────────────────────
  // Runs once on page load; only changes the CSS variable so everything using
  // var(--gold) / var(--gold-dk) automatically picks up the new colour.
  // If the admin hasn't set a custom colour yet, the default gold is kept as-is.
  (async function applyBranding(){
    try {
      var r = await API.req('GET', '/content/branding');
      if (!r || !r.data) return;
      if (r.data.primaryColor && /^#[0-9A-Fa-f]{3,8}$/.test(r.data.primaryColor)) {
        var hex = r.data.primaryColor;
        document.documentElement.style.setProperty('--gold', hex);
        document.documentElement.style.setProperty('--gold-dk', hex);
      }
      if (r.data.logoUrl) {
        // Replace the SVG emblem with the custom logo image
        var embl = document.querySelector('.hdr-emblem');
        if (embl) {
          var img = document.createElement('img');
          img.src = r.data.logoUrl;
          img.alt = 'Logo';
          img.style.cssText = 'width:28px;height:28px;object-fit:contain;border-radius:6px;';
          img.onerror = function(){ img.remove(); }; // silently keep SVG if img fails
          embl.innerHTML = '';
          embl.appendChild(img);
        }
      }
    } catch(e) { /* fail silently — fallback to default gold */ }
  })();
</script>
</body>
</html>
